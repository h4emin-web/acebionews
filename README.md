...
│  │  │ crawl-deals │  │ generate-    │  │ search-manufacturers │  │ │
│  │  │ crawl-ibric │  │  intelligence│  │ search-external      │  │ │
│  │  │ crawl-cnn   │  │ generate-    │  │ pharma-chat          │  │ │
│  │  │ crawl-mfds  │  │  daily-brief │  │                       │  │ │
│  │  │ crawl-      │  │ translate-   │  └───────────────────────┘  │ │
│  │  │  regulatory │  │  news        │                              │ │
│  │  │ crawl-      │  │ enrich-nce-  │  ┌───────────────────────┐  │ │
│  │  │  clinical   │  │  patents     │  │ ⏰ Cron Scheduler     │  │ │
│  │  │ crawl-      │  │              │  │  (pg_cron 자동 실행)  │  │ │
│  │  │  reports    │  │              │  │                       │  │ │
│  │  └──────┬──────┘  └──────┬───────┘  └───────────────────────┘  │ │
│  └─────────┼───────────────┼──────────────────────────────────────┘ │
...
└──────────────────────────────────────────────────────────────────────┘
              ...
              ▼                ▼                ▼
   ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
   │ 🌐 RSS Feeds │  │ 🔬 Gemini AI │  │ 🏥 공공 API  │
   │ FiercePharma │  │ 뉴스 분석    │  │ FDA / MFDS   │
   │ BioPharma    │  │ 인텔리전스   │  │ 임상시험 DB  │
   │ PharmaTimes  │  │ 번역         │  │ 의약품 정보  │
   └──────────────┘  └──────────────┘  └──────────────┘
```
...
───────────         ──────────────────           ────────────            ────────────────
 ...
 RSS Feeds ────┐
               │    ┌──────────────────┐    ┌──────────────┐    ┌──────────────────┐
 HTML 크롤링 ──┼───▶│  crawl-news      │───▶│ news_articles│───▶│  뉴스 피드       │
               │    │  (Cron: 매일)    │    └──────────────┘    │  필터/검색       │
 Substack ─────┘    └──────────────────┘            │           │  키워드 하이라이트│
                                                    ▼           └──────────────────┘
                    ┌──────────────────┐    ┌──────────────┐
                    │  analyze-news    │───▶│ AI 분석 결과 │───▶ 비즈니스 임팩트 표시
                    │  (Gemini API)    │    └──────────────┘
                    └──────────────────┘
                                           ┌──────────────┐
                    ┌──────────────────┐    │ intelligence │
                    │generate-         │───▶│ _summaries   │───▶ 일일 인텔리전스 요약
                    │ intelligence     │    └──────────────┘
                    └──────────────────┘

 FDA.gov ───────────┐                      ┌──────────────┐
                    ├─────────────────────▶│ regulatory   │───▶ 규제 동향 대시보드
 MFDS ─────────────┘                      │ _notices     │
                                          └──────────────┘
```
...
│ article_id       │     │ keyword           │     │ patent_expiry_date   │
│ memo             │     │ created_at        │     │ company              │
│ folder_id        │     └───────────────────┘     │ indication           │
└──────────────────┘                                └──────────────────────┘
